...
$con->update('bpu')->set_value_update('is_locked', true)
    ->whereRaw("((DATEDIFF(NOW(), bpu.waktustempel)) -
            ...
            (case when weekday(bpu.waktustempel) = 5 then 1 else 0 end)) > 3 AND status = 'belum Di Bayar' AND ((checkby IS NULL OR checkby = '') OR (approveby IS NULL OR approveby = ''))")->save_update();
