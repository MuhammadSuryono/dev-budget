...
require "application/config/whatsapp.php";
require "application/config/message.php";
...
if (isset($_POST['submit'])) {
  ...
  $email = [];
  $nama = [];
  $queryEmail = mysqli_query($koneksi, "SELECT phone_number,nama_user FROM tb_user WHERE nama_user = '$bpu[pengaju]' AND aktif='Y'");
  ...
  $queryEmail = mysqli_query($koneksi, "SELECT phone_number,nama_user,divisi FROM tb_user WHERE nama_user = '$s[pengaju]' AND aktif='Y'");
  $emailUser = mysqli_fetch_assoc($queryEmail);
  if ($emailUser) {
    array_push($email, $emailUser['phone_number']);
    array_push($nama, $emailUser['nama_user']);
  ...
  if ($selesai['status'] == 'Vendor/Supplier' || $selesai['status'] == 'Honor Eksternal') {
    ...
    if ($selesai['status'] == 'Vendor/Supplier') {
      ...
      ";
      // $tglInvoice = $explodeString[2][0] . $explodeString[2][1] . "/" . $explodeString[2][2] .  $explodeString[2][3] . "/20" . $explodeString[2][4] . $explodeString[2][5];
      // $term = $explodeString[3][1] . " of " . $explodeString[3][3];

      // $msg = $helperMessage->messageStatusPembayaranBPUVendorSuplier($bpu['namapenerima'], $explodeString[1], $tglInvoice, $term, $explodeString[4], $bpu['norek'], $bank['namabank'], $jumlahbayar, $dt->format('d/m/Y'));
    } else {
      $msg = "Kepada " . $bpu['namapenerima'] . ", <br><br>
      Berikut informasi status pembayaran yang akan Anda terima:<br><br>
      ...
      // $msg = $helperMessage->messageStatusPembayranBPUNonVendor($bpu['namapenerima'], $bpu['ket_pembayaran'], $bpu['norek'], $bank['namabank'], $bpu['namapenerima'], $dt->format('d/m/Y'));
    }

    if (!is_null($bpu['emailpenerima']) && !is_null($bpu['namapenerima'])) {
      // $sqlUser = mysqli_query($koneksi, "SELECT phone_number FROM tb_user WHERE nama_user = '$bpu[namapenerima]' AND '$bpu[namapenerima]' AND email = '$bpu[emailpenerima]'");
      // $user = mysqli_fetch_assoc($sqlUser);

      // $whatsapp->sendMessage($user["phone_number"], $msg);
