  ...
  <div class="panel-body no-padding">
    ...
    <ul class="nav nav-tabs">
      <?php for ($i = $year; $i > $year - 4; $i--) :
        if ($i == $year) :
      ?>
          <li class="active"><a href="#honor<?= $i ?>"><?= $i ?></a></li>
        <?php else : ?>
          <li><a href="#honor<?= $i ?>"><?= $i ?></a></li>
        <?php endif; ?>
      <?php endfor; ?>
    </ul>
    ...
    <div class="tab-content">
      ...
      <?php for ($j = $year; $j > $year - 4; $j--) :
      ?>
        <div id="honor<?= $j ?>" class="tab-pane fade <?= ($j == $year) ? "in active" : "" ?>">
          ...
          <table class="table table-striped table-bordered">
            ...
            <tbody>
              <?php
              $i = 1;
              $carihonor = mysqli_query($koneksi, "SELECT * FROM selesai WHERE rincian LIKE '%honor%' AND waktu IN (SELECT waktu FROM pengajuan WHERE jenis='B1' AND status='Disetujui' AND tahun = '$i')");
                  ...
                  if (mysqli_num_rows($liatbayar) == 0) {
                    ...
                    while ($bayar = mysqli_fetch_array($liatbayar)) {
                      ...
                      if ($persetujuan == 'Belum Disetujui' && $statusbayar == 'Belum Di Bayar') {
                        ...
                        $color = 'orange';
                      } else if (($persetujuan == 'Disetujui (Direksi)' || $persetujuan == 'Disetujui (Sri Dewi Marpaung)') && $statusbayar == 'Telah Di Bayar' && ($exin == 'Honor Eksternal' || $exin == 'Vendor/Supplier' || $exin == 'Lumpsum' || $exin == 'Honor SHP Jabodetabek' || $exin == 'Honor SHP Luar Kota' || $exin == 'Honor SHI/PWT Jabodetabek' || $exin == 'Honor SHI/PWT Luar Kota' || $exin == 'Honor Telp Cabang Jabodetabek' || $exin == 'Honor Telp Cabang Luar Kota')) {
                        $color = '#d5f9bd';
