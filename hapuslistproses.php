...
if (isset($_POST['submit'])) {
  ...
  $update = mysqli_query($koneksi, "UPDATE pengajuan SET status = 'Dihapus' WHERE waktu ='$time'");
  ...
  if ($update) {
    ...
    $msg = "Dear $pengaju, <br><br>
        ...
        Telah di Hapus oleh <strong> $pembuat </strong> pada <strong> " . date("d/m/Y H:i:s") . "</strong><br><br>
        ";

    $msg .= "Klik <a href='$url'>Disini</a> untuk membuka aplikasi budget.";
    $subject = "Notifikasi Untuk Penghapusan Budget";
    if ($email) {
      $message = sendEmail($msg, $subject, $email, $name, $address = "multiple");
    }
    $notifikasi = 'Budget Telah Di Hapus. Pemberitahuan via email telah terkirim ke ';
    ...
    // $notifikasi = "Budget Telah Di Hapus. Pemberitahuan via email telah terkirim ke $pengaju ($email)";
